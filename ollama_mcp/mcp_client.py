...
async def call_tool(
    ...
    try:
        ...
        # Tool raised an exception server-side.  Return the error message so
        # the model can see what went wrong instead of getting empty input.
    ...
    text = "\n".join(parts)
    # FastMCP returns content=[] (no blocks) when the tool returns an empty
    # list.  Give the model a clear signal in that case rather than an empty
    # string that it might silently ignore.
