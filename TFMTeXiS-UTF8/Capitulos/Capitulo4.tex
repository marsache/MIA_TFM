...
\begin{figure}[H]
    ...
    \node[box, below=of user] (cliente)
    {Cliente MCP};

    \node[box, below=of cliente] (columnas)
    {Recuperación semántica\\de columnas};

    \node[box, below=of columnas] (esquema)
    {Mini esquema relevante};

    \node[box, below=of esquema] (llm)
    {Llama 3.1 8B\\(Ollama)};

    \node[box, below=of llm] (tool)
    {Tool Calling MCP};

    \node[box, below=of tool] (servidor)
    {Servidor MCP};

    \node[box, below=of servidor] (obtener)
    {\texttt{obtener\_esquema()}};

    \node[box, below=of obtener] (buscar)
    {\texttt{buscar\_columnas\_relevantes()}};
    
    \node[box, below=of buscar] (sql)
    {\texttt{ejecutar\_consulta\_sql()}};

    \node[box, below=of sql] (sqlite)
    {SQLite};
