...
El método utilizado para esta extracción de tempo es \verb|_extract_bpm|.

El método recorre la estructura jerárquica del objeto musical en busca de elementos de tipo \verb|MetronomeMark|, los cuales contienen información explícita sobre la velocidad de ejecución expresada en pulsaciones por minuto (BPM). Dado que una obra puede presentar múltiples cambios de tempo a lo largo de su desarrollo, el algoritmo selecciona la primera marca de metrónomo válida encontrada durante el recorrido de la partitura.
...
El método utilizado para esta extracción de compás es \verb|_extract_compas|.
...
El método principal utilizado para esta extracción de la tonalidad es \verb|_extract_tonality|.
...
El método utilizado para esta extracción de metadatos es \verb|_extract_title_and_author|.
...
El método utilizado para esta inferencia del modo es \verb|_infer_mode_custom|.
...
El método utilizado para esta detección de hemiolias verticales es \verb|detectar_hemiolas_verticales|.
...
El método utilizado para esta detección de hemiolias horizontales es \verb|detectar_hemiolas_horizontales|.
...
El método utilizado para esta detección de síncopas es \verb|detectar_sincopas|.
...
El método principal utilizado para esta extracción es \verb|_extract_lyrics|.
...
El método utilizado para este análisis es \verb|_analizar_temas_con_ollama|.
...
El método utilizado para esta detección es \verb|_detectar_cambio_resolucion_ppq|.
...
El método utilizado para esta detección es \verb|detectar_desajustes_meter_sig|.
...
El método utilizado para esta detección es \verb|detectar_valores_irregulares_ocultos|.
...
El método utilizado para esta extracción de densidad de eventos es \verb|_calcular_densidad_eventos|.
...
El método utilizado para esta detección de polirritmias es \verb|detectar_polirritmias|.
...
El método utilizado para esta extracción de tesitura es \verb|_calcular_tesitura|.
...
El método utilizado para esta inferencia es \verb|_clasificar_genero_autor|.
...
El método utilizado para este análisis es \verb|_analizar_perspectiva_lirica|.
...
El método utilizado para esta extracción es \verb|_extract_notes_and_rests|.
...
El método utilizado para esta extracción es \verb|_extract_melody_pitches|.
...
El método utilizado para este análisis es \verb|_analyze_interval_directions|.
...
El método utilizado para esta clasificación es \verb|_determine_predominant_trend|.
...
El método utilizado para esta extracción es \verb|_extract_syllable_duration_mapping|.
...
El método utilizado para esta extracción de metadatos es \verb|_extract_transcription_metadata|.
...
El método utilizado para evaluar estructuralmente la partitura es \verb|_extract_quality_control_metadata|.
...
El método utilizado para identificar el volumen MIDI de la obra es \verb|_extract_melodic_intervals|.
...
El método utilizado para identificar el volumen MIDI de la obra es \verb|_detect_leitmotivs_ngrams|.
...
El método utilizado para identificar el volumen MIDI de la obra es \verb|_extract_midi_volume|.
