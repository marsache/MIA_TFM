...
def build_clean_content(doc: Document) -> str:
    ...
    return f"""
TÍTULO: {titulo}
NOMBRE DE LA CANCIÓN: {titulo}
CANCIÓN: {titulo}

FUENTE_TITULO: {fuente}

{titulo} es una canción del corpus.
...
def build_rag_pipeline(root_dir='../corpus/datasets/para_nlp'):  
    ...
    for doc in documents:
        titulo = doc.metadata.get("titulo", "").lower().strip()
        if titulo:
            title_index[titulo] = doc
...
""")
    ...
    rag_pipeline = (
    {
        "context": lambda q: hybrid_retrieve(q, retriever, title_index),
